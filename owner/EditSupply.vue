  ...
  module.exports = {
    ...
    methods : {
      ...
      get_supply : function(){
        ...
        let conf = { headers: { "Api-Token" : this.key} };
        axios.get(base_url + "/supply/get/" + this.supply.id_supply, conf)
        .then(response => {
          this.$bvToast.hide("loading");
          this.supply.id_supply = response.data.supply[0].id_supply;
          this.supply.id_supplier = response.data.supply[0].id_supplier;
          let date = new Date(response.data.supply[0].waktu);
          let currentDate = date.toISOString().slice(0,10);
          this.supply.date = currentDate;
          this.supply.time = (date.getHours() < 10 ? "0"+date.getHours() : date.getHours())
          + ":" + (date.getMinutes() < 10 ? "0"+date.getMinutes() : date.getMinutes());

          this.supply.id_users  = response.data.supply[0].id_users;
          this.supply.total_bayar = response.data.supply[0].total_bayar;
          this.detail_supply = response.data.supply[0].detail_supply;
        })
        .catch(error => {
          alert(error);
        })
      ...
      Save : function(){
        ...
        if (this.detail_supply.length > 0) {
          if (confirm("Apakah Anda yakin menyimpan data supply ini?")) {
            ...
            axios.post(base_url + "/supply/update", form, conf)
            .then(response => {
              this.$bvToast.hide("loading");
