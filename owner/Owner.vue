  ...
  module.exports = {
    ...
    methods: {
      get_users : function() {
        ...
        axios.get(base_url+"/owner/"+this.perPage+"/"+offset, conf)
        .then(response => {
      ...
      find : function() {
        ...
        axios.post(base_url+"/owner/"+this.perPage+"/"+offset, form, conf)
        .then(response => {
          this.$bvToast.hide("loading");
          this.user = response.data.owner;
          this.totalRow = response.data.count;
      ...
      Save : function(){
        ...
        if (this.action === "insert") {
          useUrl = base_url + "/owner/save";
        }else{
          useUrl = base_url + "/owner/update";
        }
      ...
      Drop : function(id){
        ...
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
          this.$bvToast.show("loading");
          axios.delete(base_url + "/owner/drop/"+id, conf)
          .then(response => {
            this.message = response.data.message;
            this.find();
            this.$bvToast.hide("loading");
