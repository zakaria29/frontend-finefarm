  ...
  module.exports = {
    ...
    methods: {
      get_users : function() {
        ...
        axios.get(base_url+"/cashier/"+this.perPage+"/"+offset, conf)
        .then(response => {
          this.$bvToast.hide("loading");
          this.user = response.data.cashier;
      ...
      find : function() {
        ...
        axios.post(base_url+"/cashier/"+this.perPage+"/"+offset, form, conf)
        .then(response => {
          this.$bvToast.hide("loading");
          this.user = response.data.cashier;
          this.totalRow = response.data.count;
      ...
      Save : function(){
        ...
        if (this.action === "insert") {
          useUrl = base_url + "/cashier/save";
        }else{
          useUrl = base_url + "/cashier/update";
        }
      ...
      Drop : function(id){
        ...
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
          this.$bvToast.show("loading");
          axios.delete(base_url + "/cashier/drop/"+id, conf)
          .then(response => {
            this.message = response.data.message;
            this.find();
            this.$bvToast.hide("loading");
